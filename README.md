...
1. Clean your excel dataset by removing the duplicates from it (Data Tab → Data tools group → Remove duplicates)
2. Create a database in SQL and give it a name (I named it 'Ola').
3. Now in this new database, create a table in query tool
...
4. Now Import data into this table by right clicking on the table and selecting import/export and properly navigating the options.
If it shows any error due to the fact that it was set up to "add a comma after every field" without a special rule to skip the very last one.

Use the following code to add another column for it:

ALTER TABLE Bookings
ADD COLUMN Extra_Column TEXT;

This will add an empty column for the extra commas present in your csv file.

5. Check the data you imported by running the following functions to see the data:

SELECT * FROM Bookings; ,
SELECT DISTINCT Extra_Column FROM Bookings; ,
etc.

6. Now lets solve the following business related questions by performing Sql operations and EDA.
...
--Run the above bit first to save the view and THEN do the below one to see the data
...
7. With that we've done all the exploratory analysis, thereby giving answers to all the necessary business questions.
